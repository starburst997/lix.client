...
"9DRxrRFDf74evWhCgkfoLnNYMHLOYCOWREbXl8TC/YA=":
function (require, module, exports, __dirname, __filename) {
						...
						return tink_core__$Future_Future_$Impl_$.async(function(cb1) {
							...
							connect = function(attempt) {
								...
								cnx.on("error",function(e) {
									if(attempt >= max) {
										var connect1 = tink_core_Outcome.Failure(new tink_core_TypedError(null,"Failed to connect to 127.0.0.1:" + port + " after " + max + " attempts because " + e,{ fileName : "haxeshim/CompilerServer.hx", lineNumber : 272, className : "haxeshim.CompilerServer", methodName : "connect"}));
										cb1(connect1);
									} else {
										var f3 = connect;
			...
			if(_hx_tmp1.length == 2) {
				if(_hx_tmp1[0] == "--run") {
					...
					if(haxeShimExtension.indexOf("-") != -1 && haxeShimExtension.toLowerCase() == haxeShimExtension) {
						...
						var _g11 = args3.indexOf("--connect");
						if(_g11 == -1) {
							var tmp2 = _g2 == -1;
						} else if(_g2 == -1) {
					...
					var _g12 = args5.indexOf("--connect");
					if(_g12 == -1) {
						var tmp3 = _g3 == -1;
					} else if(_g3 == -1) {
				...
				var _g13 = args7.indexOf("--connect");
				if(_g13 == -1) {
					var tmp4 = _g4 == -1;
				} else if(_g4 == -1) {
